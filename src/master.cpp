...
int GFSMaster::assign_chunk(const std::string& ip_addr, chunkid_t cid, bool primary){
    auto c = grpc::CreateChannel(ip_addr, grpc::InsecureChannelCredentials());
    auto client = std::move(gfs::CServer::NewStub(c));
...
void GFSMaster::get_servers(std::vector<std::string>& servers){
    ...
    std::vector<cs_kv_t> vec(this->chunk_servers.begin(), this->chunk_servers.end());
    ...
    for(const auto&x : vec)
        servers.push_back(x.first);
}

grpc::Status GFSMaster::HeartBeat(grpc::ServerContext *ct, const gfs::HBPayload *p, gfs::Status* r){
    time_t ts = std::time(0);
    std::cout << ts << ": " << p->id() << " has "
              << p->numchunks() << " chunks" << std::endl;
...
grpc::Status GFSMaster::Open(grpc::ServerContext *ct, const gfs::OpenPayload *p, 
    ...
    std::cout << "Open(" << p->id() << ", " << p->filename()
              << ", " << p->mode() << ")" << std::endl;

    std::lock_guard<std::mutex> g(this->chunks_mutex);
    if (this->chunks.find(file_name) == this->chunks.end() && (mode & (READ|APPEND))){
...
grpc::Status GFSMaster::RequestChunk(grpc::ServerContext *ctx, const gfs::FileHandle *fh,
                                     gfs::RCResp *r){
    chunkid_t cid;
    bool new_file = false;
    gfs::ChunkServer *loc;
    ...
    std::cout << "RequestChunk(" << file_name << ", " << chunk_index << ")" << std::endl;
